...
From `weather_flow/`, create a `.env` file (do **not** commit it):
...
- **Processed events**: `weather_flow/data/processed/`
- **Monitoring logs (JSONL)**: `weather_flow/data/monitoring/`
...
WeatherFlow logs streaming metrics in `weather_flow/data/monitoring/stream_metrics.jsonl` as JSON Lines. Each record includes:
